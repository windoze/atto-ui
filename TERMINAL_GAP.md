...
**改动点**：
- 组件层：capture 分支引入「前缀态」状态机——收到前缀键则进入前缀态（不转发），下一个键查前缀命令表；命中外壳命令则 `return EventResult::ignored()` 让事件冒泡给 Desktop，命中 `[` 则进 copy-mode，`前缀+前缀` 则转发一个字面前缀，未命中则（可选）连同前缀一起发给子进程或吞掉。前缀键、命令表可配置。
