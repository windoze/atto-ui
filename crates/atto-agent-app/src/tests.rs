...
use crate::deepseek::{
    ...
    ChatFunctionCallDelta, ChatMessageRole, ChatToolCallDelta, ChatToolKind, ToolChoice,
    ToolChoiceMode, chat_error_from_http_status, chat_error_from_json_error,
    chat_error_from_network_failure, chat_error_from_stream_disconnect, parse_chat_completion_sse,
...
use crate::tool::{
    ToolContext, ToolExecutor, ToolOutputKind, ToolPermission, ToolPermissionPolicy, ToolRegistry,
    ToolResult, ToolSpec,
};

use super::{
    ACCEPTED_PLAN_EXECUTION_INSTRUCTION, APP_TITLE, AgentApp, AgentTurnLauncher, AgentTurnLimits,
    AppAction, MISSING_DEEPSEEK_API_KEY_NOTICE, MockTurnRegistry,
    PLAN_MODE_MUTATING_TOOL_BLOCKED_RESULT, PlanDecisionRuntime, STATUS_READY, STATUS_STREAMING,
    SlashRuntime, StatusSegmentBindings, ToolRuntime, TranscriptPersistence, TranscriptStatusState,
    TurnBudgetTracker, append_startup_notices, apply_app_action, build_chat_panel,
...
fn test_skill_registry(skills: &[(&str, &str, &str)]) -> (PathBuf, SkillRegistry) {
    ...
    let registry = SkillRegistry::discover_from_paths(&[SkillSearchPath::workspace(&workspace)]);
    (workspace, registry)
}
...
fn live_http_error(status: u16, reason: &'static str, body: &'static str) -> LiveHttpErrorResult {
...
fn apply_test_app_action(
    ...
    let tool_runtime = test_tool_runtime(AgentConfig::defaults("."), sender, registry, permissions);
    let transcript_status = TranscriptStatusState::new();
    apply_app_action(
        store,
        input_handle,
...
impl ToolExecutor for SlowTool {
    ...
    fn execute(&self, _ctx: ToolContext, _args: serde_json::Value) -> anyhow::Result<ToolResult> {
...
fn accept_with_timeout(
    ...
    loop {
        match listener.accept() {
            ...
            Err(error)
                if error.kind() == std::io::ErrorKind::WouldBlock && start.elapsed() < timeout =>
            {
...
fn transcript_status_summarizes_tokens_and_latest_error() {
    let store = ChatMessageStore::new();
    let token_estimate_state = atto_ui::reactive::Property::new(format_token_estimate_status(0));
...
fn deepseek_provider_continues_after_denied_live_tool_call() {
    ...
    let tool = store
        ...
        .flat_map(|message| message.blocks.iter())
        .find_map(|block| match block {
            ChatBlock::ToolUse(tool) if tool.call_id == "call_denied_command" => Some(tool.clone()),
            _ => None,
        })
...
fn deepseek_error_mapping_covers_http_network_disconnect_and_json_failures() {
    ...
    let mut stream = new_test_stream();
    let error =
        single_failed_error(stream.map_error(chat_error_from_http_status(429, "rate limit body")));
    ...
    let mut stream = new_test_stream();
    let error =
        single_failed_error(stream.map_error(chat_error_from_network_failure("request timed out")));
    assert_eq!(error.kind, ChatErrorKind::Network);
    assert!(
        error
            .detail
    ...
    let mut stream = new_test_stream();
    let error =
        single_failed_error(stream.map_error(chat_error_from_json_error(json_error, "{not json")));
    assert_eq!(error.kind, ChatErrorKind::Api);
...
fn deepseek_stream_error_event_fails_turn_with_structured_detail() {
    ...
    let branch = store.branch_token();
    let _cancel = mock_turns.start(assistant_id);
    let mut stream = DeepSeekUiStream::new(branch, assistant_id, text_block_id, "deepseek-chat");
