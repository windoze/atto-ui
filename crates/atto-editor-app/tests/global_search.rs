...
#[test]
fn global_search_ctrl_shift_f_finds_todo_and_opens_file() -> anyhow::Result<()> {
    let root = unique_temp_dir("global_search");
    fs::create_dir_all(root.join("src"))?;
    let file = root.join("src").join("main.rs");
    fs::write(&file, "fn main() {}\n// TODO: open from global search\n")?;
    ...
    host.wait_for_text("Global Search", PTY_WAIT)?;
    host.send_str("TODO")?;
    host.key_with_mods(KeyCode::Enter, KeyModifiers::NONE)?;
    host.wait_for_text("src/main.rs:2:4", PTY_WAIT)?;
    host.key_with_mods(KeyCode::Enter, KeyModifiers::NONE)?;
    host.wait_for_text("open from global search", PTY_WAIT)?;
