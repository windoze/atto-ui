...
- [x] **[DONE] P8.1 Plan 模式展示+接受** — 新增独立 plan block 模型和渲染,不要用 Todo/Approval 组合替代。`src/message.rs` 增加 `PlanBlock`/`PlanItem`/`PlanDecision`;`src/store.rs` 增加 plan 更新与决策 API;`src/list.rs` 渲染 plan 面板并在 pending 时显示 Accept/Reject,决策后锁定;`src/dynamic.rs`、`packages/core`、`packages/react`、`docs/NODE_API.md` 同步新形。`snapshot_chat_app --plan-mode` + PTY 覆盖展示、接受事件和锁定状态。
  - 完成记录（2026-06-12）：已新增独立 `PlanBlock`/`PlanItem`/`PlanDecision` 模型，并从 `lib.rs` 公开导出；`ChatMessageStore` 增加 `set_plan` 与 `set_plan_decision`，保持未变化不触发脏通知。`ChatMessageList` 新增 plan 面板渲染、pending Accept/Reject、决策后锁定展示和 `on_plan_decision` 回调；`dynamic.rs` 同步 `plan` block 序列化/解析和 `plan_decision(Map)` 事件。`snapshot_chat_app --plan-mode` 与 PTY 覆盖展示、接受事件和锁定状态；`packages/core`/`packages/react`/`crates/atto-ui-node/index.d.ts`/`docs/NODE_API.md` 已同步 `ChatPlanBlock`、`ChatPlanDecision` 和 `onPlanDecision`。
  - 验证：`cargo fmt --all`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets`、`npm run typecheck --prefix packages/core`、`node packages/core/__test__/builders.cjs`、`npm test --prefix packages/core`、`npm run test:runtime --prefix packages/core`、`npm run typecheck --prefix packages/react`、`npm run build --prefix packages/react`、`npm test --prefix packages/react`、`npm test --prefix crates/atto-ui-node`、`npm run smoke --prefix examples/react-tsx` 全部通过。
- [x] **[DONE] P8.2 子 agent / Task 嵌套块** — 新增显式 task/subagent block,支持在 assistant 回合内展示一个可折叠的子 agent 运行摘要和嵌套 transcript/blocks,不要把它建模成普通 tool output 文本。同步 store 定点更新、dynamic/TS 类型、React builders 和文档。`snapshot_chat_app --nested-task` + PTY 覆盖折叠/展开、嵌套内容渲染、状态更新和虚拟化下可见窗口行为。
  - 完成记录（2026-06-12）：已新增显式 `TaskBlock` / `TaskStatus` / `TaskTranscriptItem` 模型，task block 持有状态、摘要和带 role 的嵌套 transcript blocks；`ChatMessageStore` 增加 task 状态、摘要、transcript 块级更新 API，并保持未变化不发脏通知。`dynamic.rs` 支持 `task` block 递归序列化/解析；`ChatMessageList` 以 `Disclosure` 渲染可折叠 task 摘要和嵌套 transcript 内容，状态更新通过块版本同步，虚拟化仍只实现可见窗口行。`snapshot_chat_app --nested-task` 与 PTY 覆盖折叠/展开、嵌套内容、状态更新和滚动后可见窗口行为；`packages/core`/`packages/react`/`crates/atto-ui-node/index.d.ts`/`docs/NODE_API.md` 已同步 task block builders、类型和文档。
  - 验证：`cargo fmt --all`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets`、`npm run typecheck --prefix packages/core`、`node packages/core/__test__/builders.cjs`、`npm test --prefix packages/core`、`npm run typecheck --prefix packages/react`、`npm run build --prefix packages/react`、`npm test --prefix packages/react`、`npm test --prefix crates/atto-ui-node`、`npm run smoke --prefix examples/react-tsx` 全部通过。
- [x] **[DONE] P8.3 聊天文本选择** — 在 `ChatMessageList` 的文本/代码/命令目标 block 内实现真实文本选择,与已有 Copy/CopyBlock 动作并存；支持鼠标拖选和复制所选文本,选择范围跨软换行时保持显示宽度正确。补单元测试和 `snapshot_chat_app --text-selection` PTY 覆盖选区渲染、复制所选文本、未选择时仍触发 CopyBlock。
  - 完成记录（2026-06-12）：已在 `ChatMessageList` 的 `BlockCopyTarget` 中加入基于渲染行的真实选区，支持鼠标拖选、选区高亮、宽字符整字宽选择和跨视觉行复制；有选区时 `Ctrl/Cmd+C` 通过 OSC52 复制所选文本，无选区时保持既有 `CopyBlock(block_id)` 回调。新增 `snapshot_chat_app --text-selection` 场景覆盖文本、代码和命令目标 block，并补 PTY 验证选区渲染、复制所选文本、无选区 fallback。
  - 验证：`cargo fmt --all`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo test -p atto-ui-chat selection`、`cargo test -p atto-ui-chat --test pty_chat chat_text_selection_highlights_copies_selection_and_falls_back -- --exact`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets` 全部通过；未发现 `tools/run_fixtures.py` 夹具入口。
...
- [x] **[DONE] 收尾 2 — 快照人工比对** — 依赖 P8.1、P8.2、P8.3。用 `snapshot_chat_app` 抓屏,逐项核对 `CHAT_UI.md` §2 能力矩阵从 ❌/⚠️ 转为 ✅。
  - 完成记录（2026-06-12）：已在 P8.1-P8.3 完成后重新执行 `snapshot_chat_app` PTY 快照审计，临时审计辅助测试抓取并人工核对 18 组代表性屏幕：streaming markdown、block mapping top/bottom、long tool output tail/expanded、inline approval、inline diff、plan accepted、nested task complete、thinking/notices、todo updated、turn meta/error、message actions、text selection copied、cancel action、responsive narrow/wide、artifact links。审计未发现新的 spec 缺口；`CHAT_UI.md` §2 已更新为最终能力覆盖矩阵，所有原 ❌/⚠️ 项均标为 ✅ 并记录对应实现/验证点。
  - 验证：临时 `cargo test -p atto-ui-chat --test pty_chat_audit -- --nocapture` 审计通过（临时测试随后删除）；`cargo fmt --all -- --check` 通过；`cargo test -p atto-ui-chat --test pty_chat` 23 项通过。最终保留改动仅为文档/进度文件，`cargo build --workspace --all-targets`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo test --all --all-targets` 复用 P8.3/收尾 1 的上一轮全量绿色结果，未重复运行。
