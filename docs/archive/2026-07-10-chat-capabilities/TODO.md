...
- [x] **[DONE] P6.2 权限层级渲染** — `src/list.rs`：审批区渲染分层选项（一次允许/始终允许/项目级/拒绝等），选择后状态锁定并显示已选层级。
  - 完成记录（2026-07-10）：`ChatMessageList` 的 tool approval 渲染现在基于结构化 `ApprovalAction` / `ApprovalLevel` 为未决按钮补充权限范围标记，覆盖 once / always / project / deny；若宿主标签本身已经表达对应层级，则避免重复追加，保持既有 inline approval 视口内按钮可见。
  - 锁定状态：resolved approval 继续只显示已选项并不可聚焦，同时按 `ApprovalResolution` 的 action/level 渲染已选层级，确保选择后状态锁定且不会回显其它未选选项。
...
- [x] **[DONE] P6.3 上下文压缩块** — `src/message.rs` + `src/list.rs`：新增专门的 compact 块类型（或扩展 `Notice`），展示压缩进度/前后 token/摘要，视觉区别于普通通知。
  ...
  - 验证：`cargo fmt --all`、`cargo test -p atto-ui-chat --lib`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets` 均通过。
- [x] **[DONE] P6.4 运行时/JS 侧同步** — `src/dynamic.rs`：模型变更同步序列化 + schema（保留旧形兼容）；同步 `crates/atto-ui-node`、`packages/core`、`packages/react` 类型/构造器，更新 `docs/NODE_API.md`；跑 smoke 与 runtime 兼容测试。
  - 完成记录（2026-07-10）：Rust dynamic 层保留旧 approval `resolved` 字符串兼容，并通过新增单测固定结构化 `action` / `level` 序列化、旧 id/label-only option 推断，以及 compact block round-trip 覆盖；`crates/atto-ui-node/index.d.ts` 与生成后补丁脚本同步新增 `ChatApprovalAction`、`ChatApprovalLevel`、`ChatApprovalDecisionValue`、`ChatCompactStatus` 和 `compact` block 类型。
  - JS/React 同步：`packages/core` 新增 `ChatApprovalOption`、`ChatApprovalRequest`、`ChatCompactBlock` builders 和对应 TS 类型；`packages/react` re-export 新 builders/types，`useChatMessages().resolveApproval` 现在按结构化 option action/level 推进状态并写回 `resolved_action` / `resolved_level`，缺失结构化字段时仍按旧 id/label 推断；`docs/NODE_API.md` 已记录 approval 层级字段、compact block、React hook 行为和 `onApprove` payload。
  - 测试覆盖：新增/更新 core builder 和 TS 类型测试、React TSX 类型测试、Rust dynamic approval 兼容单测；raw Node build 后生成文件保持稳定。
  - 验证：`cargo fmt --all`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets`、`npm run typecheck --prefix packages/core`、`npm run typecheck --prefix packages/react`、`npm test --prefix packages/core`、`npm run test:runtime --prefix packages/core`、`npm test --prefix crates/atto-ui-node`、`npm run build --prefix crates/atto-ui-node`、`npm test --prefix crates/atto-ui-node`、`npm run build --prefix packages/react`、`npm test --prefix packages/react`、`npm run smoke --prefix examples/react-tsx`、`git diff --check` 均通过。
- [x] **[DONE] P6.5 快照与测试** — `snapshot_chat_app` 增加多层级审批与压缩块场景；PTY 覆盖层级审批选择/锁定、压缩块渲染。
  - 完成记录（2026-07-10）：`snapshot_chat_app` 新增 `--p6-approval-compact` 确定性 fixture，覆盖一次允许、始终允许、项目级允许、拒绝四类审批选项，以及独立 compact block 的 complete 状态、压缩前后 token 和多行摘要。
  - 测试覆盖：新增 `chat_p6_approval_levels_emit_and_lock_project_scope` 与 `chat_p6_compact_block_renders_tokens_and_summary` 两个 PTY 用例，分别验证项目级审批选择后事件 payload 携带 `action=allow` / `level=project`、审批 UI 锁定只显示已选项，以及 compact block 渲染 `Context compact`、token 节省量和摘要详情且不退化为普通 Notice。
  - 验证：`cargo fmt --all`、`cargo test -p atto-ui-chat --test pty_chat chat_p6 -- --nocapture`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets` 均通过。
- [x] **[DONE] P6.R Review：P6 阶段复核** — 逐条复核 P6.1–P6.5：确认权限层级语义与回调契约一致、旧形序列化仍可解析、压缩块与 Notice 区分清晰、JS 侧类型同步无遗漏；确认 PTY + smoke 全过；跑通全套 CI。做一次整体收尾比对，确认 `AGENT_GAP.md` 中除 B2（图片）外的缺口均已落地。
  - 完成记录（2026-07-10）：已逐条复核 P6.1–P6.5。Rust 模型中 `ApprovalOption` / `ApprovalResolution` 的 action/level 与 store 状态推进一致，list 审批按钮和锁定态显示 once/always/project/deny 层级，`approve` 回调 payload 携带 `action` / `level`；`CompactBlock` 作为独立块渲染状态、前后 token、节省量和多行摘要，搜索/引用/动态 round-trip 路径均覆盖，不退化为普通 `Notice`。
  - 复核修复：发现 React `useChatMessages().resolveApproval` 的旧形 deny 推断未完全对齐 Rust `ApprovalOption::from_legacy`，`label: "No thanks"` 会误判为 allow；已改为识别任意空白分隔单词 `no`，并新增 React render 回归测试，确认旧形选项解析为 `canceled deny once`。
  - 收尾比对：已更新 `AGENT_GAP.md`，将除 B2（图片/多模态内联渲染，仍明确暂缓）外的 A1–A4、B1/B3、C1–C3、D1/D2 均标记为已落地，并补充当前 capabilities 中的 `Compact` 与权限层级说明。
  - 验证：`npm run build --prefix packages/react`、`node packages/react/__test__/render.cjs`、`cargo fmt --all`、`cargo clippy --workspace --all-targets -- -D warnings`、`cargo fmt --all -- --check`、`cargo build --workspace --all-targets`、`cargo test --all --all-targets`、`npm run build --prefix crates/atto-ui-node`、`npm exec --yes --package=typescript@5.9.3 -- tsc -p packages/core/tsconfig.json --noEmit`、`npm run typecheck --prefix packages/react`、`npm test --prefix crates/atto-ui-node`、`npm test --prefix packages/core`、`npm run test:runtime:bun --prefix packages/core`、`npm run test:runtime:deno --prefix packages/core`、`npm test --prefix packages/react`、`npm run smoke --prefix examples/react-tsx`、`npm run npm:artifacts --prefix crates/atto-ui-node`、`npm pack --dry-run --json ./crates/atto-ui-node`、`npm pack --dry-run --json ./crates/atto-ui-node/npm/darwin-arm64`、`npm pack --dry-run --json ./packages/core`、`npm pack --dry-run --json ./packages/react`、`git diff --check` 均通过。本机为 darwin-arm64，Linux 平台子包 dry-run 仍由 Ubuntu CI artifact 流程覆盖。
