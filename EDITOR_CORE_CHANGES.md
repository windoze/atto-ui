...
- LSP 会话优雅关闭：0.4.2 已提供 `LspSession::shutdown()/exit()/cancel_request()`
  ...
  **已在 atto 侧接入**：`LspWorkspaceBridge::close_document` 在某 (root,language) 的最后一个
  文档关闭时移除并 `session_mut().exit()` 关停共享 server（`lsp_workspace.rs retire_key_if_unused`）。
- 搜索正则重复编译：0.4.2 已提供 `editor_core::search::CompiledSearch`，atto 侧换用即可。
  **已在 atto 侧接入**：workspace 搜索改为编译一次复用（`atto-editor-app/src/search.rs`）。
